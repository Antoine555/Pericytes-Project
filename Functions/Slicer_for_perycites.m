...
peryxyz=permute([199 183 29-4],[2 1 3]);
...
for row=1:size(F3,1)
    ...
    idx = s.PixelIdxList;
    sum_region = sum(I_temp(idx));
    x_region = s.PixelList(:, 1);
    y_region = s.PixelList(:, 2);
    
    c_2D(row,1) = sum(x_region .* double(I_temp(idx))) / sum_region;
    c_2D(row,2) = sum(y_region .* double(I_temp(idx))) / sum_region;
...
Mprof=[];
figure(10)
nber_radii=20; %choose the number of radii to be measured per centrepoint
Stored_radii_para=[];
...
subplot(2,2,4)
plot(row,Areas./Ps,'g',row, Stored_radii_para2(:,1)/2,'r' )
