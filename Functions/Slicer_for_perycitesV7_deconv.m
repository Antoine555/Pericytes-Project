...
for peri=6:1:11       %size(perixyzM,1) % analyse one pericyte at a time
    ...
    perixyz(2)=round(perixyz(2));
     ...
     % exclude the pericytes that are too close to the edges of the volume
    if perixyz(3)<=5 || perixyz(3)>=(42-5)
        ...
        %% Take a new volume around the pericyte
        % take a portion of the non interpolated volume
        V0=Stack_vessels(perixyz(1)-window/2:perixyz(1)+window/2,perixyz(2)-window/2:perixyz(2)+window/2,1:end);
        Vref0=Vref00(perixyz(1)-window/2:perixyz(1)+window/2,perixyz(2)-window/2:perixyz(2)+window/2,1:end);
        S=size(V0);
        ...
        plot3 (window/2,window/2,zi,'MarkerSize',10,'MarkerFaceColor','m','MarkerEdgeColor','m','Marker','o')
        
        %% Find distances to all centrepoints to pericyte
        ...
        if minDistance>=20 % link is too far from perictyte
            ...
            imshow(MIP(perixyz(1)-window/2:perixyz(1)+window/2,...
                perixyz(2)-window/2:perixyz(2)+window/2,:));
            hold on
