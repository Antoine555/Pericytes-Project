...
for peri=1:1:14       %size(perixyzM,1) % analalyse one pericyte at a time
    ...
    disp('Interpolation of this volume started.')
    ...
    if minDistance>=20 % link is too far from perictyte
        ...
        disp(['Pericyte' num2str(peri) 'excluded as too far from'...
            ...
            ' Close open figures then press any key to go to next peri.'])
        pause
        continue
    else
    end
    ...
    for row=1:size(F3,1) %for all centrepoints
        ...
        % extract slices if using the point on smoot spline as origin:
    ...
    disp(['Slices found, press any key to recalculate the contours' ...
        ' and centoids in 2D'])
    pause
    ...
    disp(['Close all open figures and press any key to save the measures'...
        ' and go to next pericyte.'])
    pause
    ...
    Excel_Data(1:(size(F3,1)-4),((peri-1)*7+2):(peri-1)*7+4)=Stored_radii_para2;
    Excel_Data(1:(size(F3,1)-4),((peri-1)*7+5))=Areas;
    Excel_Data(1:(size(F3,1)-4),((peri-1)*7+6))=Ps;
    %Excel_Data(1:(size(F3,1)-4),((peri-1)*7+7))=kappalist(1:26);
    %Excel_Data(1:(size(F3,1)-4),((peri-1)*7+8))=taulist(1:26);
...
disp('All valid pericytes have been explored and data is being exported to Excel.')
