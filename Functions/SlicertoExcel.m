% excel exporter for slicer for perycites V4
% i.e. no plots created for this except one for the results 
...
peryxyzM=[263 658 29-4; 183 199 29-4];
...
%% Find the active contour using level set   
    ...
    perimeteri=struct('cross_section',0,'coord',zeros(60,2));
    for row=3:size(F3,1)-2
        perimeteri(row-2).cross_section=row;
        perimeteri(row-2).coord=interparc(60, perimeter(row-2).coord(:,1),...
            perimeter(row-2).coord(:,2));
        Ps (row-2)= 60*norm( ...
    ...
    Stored_radii_para2=[];
    stored_vectors2=zeros(60,2);
    for row=3:1:size(F3)-2
        stored_vetors2=perimeteri(row-2).coord-...
            [c_2D(row-2,1)*ones(60,1),c_2D(row-2,2)*ones(60,1)];
