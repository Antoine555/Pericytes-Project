...
nber_slices= 10; % number of points to be interpolated i.e.number of slices
...
for row=1:size(F3,1)
    seg_slice2D_temp(:,:,row)= slice2D_temp(:,:,row)>170;
    ...
    if emptycenter==1
        ...
        X=seg_slice2D_temp(:,:,row);
        seg_slice2D_temp(:,:,row)=(X==label);
...
%% Find intensity profile and determine edge/radius

Mprof=[];
figure(10)
nber_radii=30; %choose the number of radii to be measured per centrepoint
Stored_radii_para=[];
edge_coord=struct('cross_section',0,'coord',zeros(nber_radii,2));
% Measure the radii from these images
for row=3:1:size(F3,1)-2 %for all centrepoints
    dummy=0;
    angle_incr=2*pi/nber_radii;
    ...
    smooth_slice=smoothn(slice2D_temp(:,:,row),0.5);
    
    for dummy=0:1:nber_radii-1    %rotate until we have rotated by 2pi
        var_radius=(slice2D_size-3)*[1 0];       %unit vector aligned with the x-axis (positve)
        ...
        pixel_coord=(c_2D(row,:)+var_radius);
        prof=improfile (slice2D_temp(:,:,row),[c_2D(row,1),pixel_coord(1)],...
            [c_2D(row,2),pixel_coord(2)],slice2D_size-3);
        profs=smoothn(prof,1);
        hessian=del2(profs);
        Mprof=[Mprof, profs];
        for rad=4:1:slice2D_size-3
            if hessian(rad)>=0
                break
            else
            end
        ...
        stored_radii(dummy+1)=rad;
        edge_coord(row-2).cross_section=row;
        edge_coord(row-2).coord(dummy+1,:)=c_2D(row,:)+ (var_radius/(slice2D_size-3)*rad); %find coord of edge points
...
plot (indexOfMin, Stored_radii_para(indexOfMin,1), 'Marker','.','MarkerSize',8,...
...
perimeter=struct('cross_section',0,'coord',zeros(nber_radii+1,2));
figure (13)

for row=3:size(F3,1)-2
    perimeter(row-2).cross_section=row;
    perimeter(row-2).coord=smoothn([edge_coord(row-2).coord(1:end,:); ...
        edge_coord(row-2).coord(1,:)],0.035);
    ...
    plot(edge_coord(row-2).coord(:,1),edge_coord(row-2).coord(:,2),...
        'Marker','.','LineStyle','none','MarkerEdgeColor','r','MarkerFaceColor','r')
    plot(c_2D(row,1),c_2D(row,2),'*')
    plot(perimeter(row-2).coord(:,1),perimeter(row-2).coord(:,2),'r')
end
...
% Add image
Shapes.AddPicture([pwd '\' 'figure16.png'] ,0,1,100,290,100,100);
...
xlswrite(File,(3:1:size(F3,1)-2)','sheet2','B3');
xlswrite(File,Stored_radii_para2,'sheet2','C3');
xlswrite(File,Areas,'sheet2','F3');
xlswrite(File,Ps,'sheet2','G3');
xlswrite(File,mylink_al,'sheet1','E3')
xlswrite(File,(mylink_al/mylink_l),'sheet1','F3');
