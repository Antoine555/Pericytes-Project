...
peryxyzM=[199 183 29-4; 263 658 29-4];
...
for pery=1:1:size(peryxyzM,1)
    ...
    for row=1:1:size(F3,1)
        ...
        mu = 20; propagation_weight = .05; GAC_weight = 5000;
    ...
    imshow(MIP(peryxyzM(1,2)-window/2:peryxyzM(1,2)+window/2,...
        peryxyzM(1,1)-window/2:peryxyzM(1,1)+window/2,:));
    hold on
    ...
    %radius
    plot(row(1:30), Stored_radii_para2((1:30),1)*0.415,'r')    %plot mean
    hold on
    plot(row(1:30), Stored_radii_para2((1:30),2)*0.415,'g')    %plot max
    plot(row(1:30), Stored_radii_para2((1:30),3)*0.415)        %plot min in blue
    %hold on
    %plot (indexOfMin, Stored_radii_para2(indexOfMin-2,1), 'Marker','.','MarkerSize',8,...
    %    'MarkerFaceColor','g','MarkerEdgeColor','m')
    figure
    %Area
   [AX,H1,H2]=plotyy(row(1:30),Areas(1:30)*(0.4151^2),row(1:30),Ps(1:30)/2.5*0.4151);
    %hold on
    %plot (indexOfMin, Areas(indexOfMin-2), 'Marker','.','MarkerSize',8,...
   %     'MarkerFaceColor','g','MarkerEdgeColor','g')
xlabel('Slice number')
...
[kappalist,taulist,ttlist,nnlist,bblist,score]=frenet_robust(F4.',25,0.2);
