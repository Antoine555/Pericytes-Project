...
for peri=10:1:20      %size(perixyzM,1) % analyse one pericyte at a time
    ...
    if minDistance>=20 % link is too far from perictyte
        ...
        disp(['Pericyte ' num2str(peri) ' excluded as too far from'...
            ' nearest link.'])
        disp('Check z coordinate or increase window size.')
        disp('Going to next pericyte')
        pause (1)
        continue
    else
    end
    ...
    if indexOfMin<=5 || size(F3,1)-indexOfMin<=6
        disp(['Pericyte ' num2str(peri) ' is too close to an endpoint'...
            ...
            ' analyse these. All open windows will be closed then and' ...
            ' the next pericyte will be analysed.'])
        close all
        pause (0.5)
        continue
    else
