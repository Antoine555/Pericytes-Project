...
for peri=11:1:14       %size(perixyzM,1) % analalyse one pericyte at a time
    ...
    for row=3:1:size(F3)-2
        ...
        if d(1)==0 && d(2)==0 % cross section is on a z-plane
            ...
            xyradius(row-2)= [Stored_radii_para2(row-2,1),Stored_radii_para2(row-2,1)];
        else
            v=[-d(2), d(1), 0]/(sqrt(d(2)^2+d(1)^2));  % find v
        end
        %first direction
        vr= F3(row,:); %Start at the centroid
        vr_value=interp3(phi, vr(2),vr(1),vr(3));
        ...
        vr3_value=interp3(phi, vr3(2),vr3(1),vr3(3));
    ...
    print(fig,'-dpng');
    disp('Close all open figures and press any key to go to next pericyte.')
    pause
    ...
    Excel_Data(1:(size(F3,1)-4), peri+2)=diameter;
